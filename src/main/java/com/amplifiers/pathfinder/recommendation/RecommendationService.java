...
public class RecommendationService {
    ...
    RecombeeClient client = new RecombeeClient(databaseId, privateToken).setRegion(Region.EU_WEST);
    ...
    public void addDetailView(Integer UserId, Integer gigId) {
        try {
            System.out.println("private token - " + privateToken);
            client.send(new AddDetailView(UserId.toString(), gigId.toString()).setCascadeCreate(true));
        } catch (Exception e) {
    ...
    public void addPurchaseView(Integer GigId, Integer UserId, String recommId) {
        try {
            client.send(new AddPurchase(UserId.toString(), GigId.toString()).setCascadeCreate(true).setRecommId(recommId));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
        ...
        try {
            if (scenario != null) {
                if (userId == -1) {
                    ...
                    return client.send(new RecommendItemsToUser("dummyUser123", numItems).setScenario(scenario));
                } else {
                    return client.send(new RecommendItemsToUser(userId.toString(), numItems).setScenario(scenario));
                }
            } else {
    ...
    public RecommendationResponse getRecommendationsForItem(Integer ItemId, Integer UserId, Integer numItems, String scenario) {
        try {
            if (scenario != null) {
                // for similar items.
                return client.send(new RecommendItemsToItem(ItemId.toString(), UserId.toString(), numItems).setScenario(scenario));
            } else {
    ...
    public RecommendationResponse search(String query, Integer userId, Integer numItems) {
        try {
            return client.send(new SearchItems(userId.toString(), query, numItems).setCascadeCreate(true));
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
